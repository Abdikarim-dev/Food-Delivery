...
const LoginPopup = ({ setShowLogin }) => {
    ...
    const [data, setData] = useState({
        ...
        email: "",
        password: "",
        isAdmin: false // Add isAdmin to state
    });
    ...
    const onLogin = async (e) => {
        ...
        if (response.data.success) {
            if (response.data.isAdmin) {
                ...
                window.open('http://localhost:5174/',); // Open admin dashboard in a new tab
                // Reset the form data
                setData({
                    name: "",
                    email: "",
                ...
                setShowLogin(false);
                navigate('/'); // Redirect regular users to home or other page
            }
        } else {
